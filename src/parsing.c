...
/*   Updated: 2024/12/05 17:39:01 by ebella           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// if the length of the first line is different from the length of the other lines the map is not a rectangle return 0
// a line finish's with '\n' so the length of the line is the length of the string + 1
...
int	check_map_edges(t_parse *parse)
{
	...
	{
		if (parse->map[i][0] != '1' 
			|| parse->map[i][ft_strlen(parse->map[i]) - 2] != '1')
				return (0);
		i++;
	}
	i = 0;
